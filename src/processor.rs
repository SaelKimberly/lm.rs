...
use crate::quantization::{MutableQuantizedTensor, QuantType, Tensor, quantize, quantize_q4};
...
impl<'a> PHI3VProcessor<'a> {
    pub fn new(data: &'a [u8]) -> PHI3VProcessor<'a> {
        ...
        PHI3VProcessor { weights, args: cfg }
    }
    ...
    pub fn process(
        ...
        let cropped_img = if num_crops > 2 {
            crop_img(&transposed_image, (new_w, new_h), num_crops)
        } else {
            transposed_image
        };
    ...
    fn hd_transform(img: &[u8], width: u32, height: u32, hd_num: u32) -> (Vec<u8>, u32, u32) {
        ...
        if trans {
            ...
            std::mem::swap(&mut new_w, &mut new_h);
        }
